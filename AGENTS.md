...
- `pnpm run schema:check` — Codex strict-mode dry run of the activity JSON Schemas against the fixtures
...
- Codex commits only when explicitly asked to in the prompt.

## Windows

Hooks in `.Codex/hooks/` are bash scripts run via Git Bash. Install **Git for Windows** and make sure `jq` is on `PATH` (the hooks fall back to `node` if it is missing) for them to parse tool-call JSON. There is no non-bash fallback: without Git Bash the hooks do not run at all, so `guard.sh` will not be blocking anything.

## MCP

`.mcp.json` declares the project-scoped MCP servers: **github** (issues/PRs) and
**playwright** (app screenshots). Codex asks for approval the first time it loads
...
skip the server. Add per-user servers such as Context7 with `Codex mcp add --scope user`.
