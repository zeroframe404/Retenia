...
notes: >-
  §9 puts P10 on the "mid" tier; `smart` is this app's nearest routing role, and the
  role map (`DEFAULT_ROLES`) is where a user moves grading onto Gemini 3.7 Flash if they
  would rather pay ≈ USD 1.7 a month for it than ≈ 3.5 (§4 of 06-ai-providers.md).
  The task itself is built by `@retenia/activity-ai`; `{{task}}` is where it lands.
