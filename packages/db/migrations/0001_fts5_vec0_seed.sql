...
--   3. Seed of the five `importance_levels` (docs/spec/02-memory-system.md §7).
...
-- rows and a future sync sees one level, not one per device. `new_per_day` NULL = no
-- per-level cap (bounded only by the session budget); `desired_retention`/`max_interval_days`
-- NULL = the level is out of the queue.
...
VALUES
	...
	('01a05a43-fc00-78a6-a8cd-9c6371180c59', 'high',        0.92, 365,  2, 1, NULL, 8, 'warn_rewrite', 1788220800000, 1788220800000, NULL, 'system', 1),
